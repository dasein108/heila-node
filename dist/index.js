...
const getPeerId = async () => {
    ...
    return peerIdFromKeys(Buffer.from(peerId.publicKey, "base64"), Buffer.from(peerId.privateKey, "base64"));
};
...
async function main() {
    ...
    const libp2p = await createLibp2p({
        ...
        datastore,
        addresses: {
            announce: ["/dns4/acidpictures.ink/tcp/4444/wss"],
            listen: [
                "/ip4/0.0.0.0/tcp/5555/ws",
                "/dns4/acidpictures.ink/tcp/4444/wss",
            ],
        },
        transports: [
            tcp(),
    ...
    return createHelia({
        ...
        console.log("Helia multiaddr(s): ", helia.libp2p.getMultiaddrs().map((ma) => ma.toString()));
        return;
    });
}
void main()
